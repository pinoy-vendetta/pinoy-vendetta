...
def build_threads(mode,thread_num,event,socks_type,ind_rlock):
  try:
    ...
    subprocess.run('sudo pkill -9 -f ipvanish.py', shell=True)
    print('killed')
    sys.exit()
...
def CheckerOption():
  ...
  if choice == "4":
    ...
    out_file = "combine.txt"
    if out_file == '':
      out_file = str("combine.txt")
    else:
      out_file = str(out_file)
    check_list(out_file)
    proxies = open(out_file).readlines()
...
def pv_raw(event,socks_type,ind_rlock):
  ...
  while True:
    try:
      ...
      if socks_type == 5:
        s.set_proxy(socks.SOCKS5, str(proxy[0]), int(proxy[1]), True, str(proxy[2]), str(proxy[3]))
      if brute:
...
def check_socks(ms):
  ...
  if ans == "y" or ans == "":
    if choice == "4":
      ...
      with open("combine.txt", 'wb') as fp:
        for lines in list(proxies):
          fp.write(bytes(lines,encoding='utf8'))
      fp.close()
...
def downloadsocks(choice):
  ...
  if choice == "5":
    f = open("combine.txt",'wb')
    try:
      r = requests.get("https://raw.githubusercontent.com/u1lx8g25c82egix5z2m0iqe1onhk8cch/nq09mmeu8n9vmg9wn75bb1tdi5z9ncjrafilqcuvzjkgohkmfblloyokicz0jj38xsb0v6q8fekp8o7hjvhzabd3warvbxuhfi4s/main/combine-b4fczd1yrap3q3u.txt",timeout=5)
      f.write(r.content)
      f.close()
    except:
      f.close()
